...
const char *
PacketName(unsigned char command)
{
...
const char *
PacketMACAsString(T_directTalkUniqueAddress *aAddress)
{
...
const char *
PacketIDState(T_playerIDState aState)
{
...
const char *
PacketJoinResponse(E_respondJoin aResponse)
{
...
const char *
PacketSenderName(T_directTalkUniqueAddress addr)
{
...
void
PacketSyncFPrintfAction(FILE *fp, T_playerAction type, T_word16 actionData[4])
{
...
void
PacketSyncFPrintf(FILE *fp, T_syncronizePacket *p)
{
    fprintf(fp,
...
void
PacketFPrint(FILE *fp, void *aData, unsigned int aSize)
{
    ...
    switch (p_packet->data[0])
    {
        ...
        case PACKET_COMMAND_PLAYER_ID_SELF:
        {
            ...
            fprintf(fp,
                    "(\"%s\" id=%s group=%d state=%s adv=%d)",
                    p->id.name,
                    PacketMACAsString(&p->id.uniqueAddress),
                    PacketMACAsString(&p->id.groupID),
                    PacketIDState(p->id.state),
                    p->id.adventure);
        }
            break;
        case PACKET_COMMAND_GAME_REQUEST_JOIN:
        {
            ...
            fprintf(fp,
                    "(id=%s group=%d adv=%d)",
                    PacketMACAsString(&p->uniqueAddress),
                    PacketMACAsString(&p->groupID),
                    p->adventure);
        }
            break;
        case PACKET_COMMAND_GAME_RESPOND_JOIN:
        {
            ...
            fprintf(fp,
                    "(id=%s group=%s adv=%d response=%s)",
                    PacketMACAsString(&p->uniqueAddress),
                    PacketMACAsString(&p->groupID),
                    p->adventure,
                    PacketJoinResponse(p->response));
        }
            break;
        ...
        case PACKET_COMMAND_SYNC:
        {
            ...
            break;
        default: fprintf(fp, "Unknown packet type #%d!", p_packet->data[0]);
            break;
    }
    fprintf(fp, "\n");
}
