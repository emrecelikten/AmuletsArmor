...
T_void
StoreSetUpInventory(T_word16 x1,
                    T_word16 y1,
                    T_word16 x2,
                    T_word16 y2,
...
T_void
StoreAddItem(T_word16 objectToAdd, T_word16 numberToAdd)
{
    T_word16 i;
...
T_void
StoreRemoveItem(T_word16 objectToRemove, T_word16 numberToRemove)
{
    T_word16 i;
...
/* alternate inventory */
T_void
StoreHandleTransaction(E_mouseEvent event,
                       T_word16 x,
                       T_word16 y,
...
T_void
StoreRequestSellItem(T_inventoryItemStruct *p_inv)
{
    ...
    if (G_houseMode == FALSE)
    {
        ...
        if (EffectPlayerEffectIsActive(PLAYER_EFFECT_GOD_MODE) == FALSE)
        {
            ...
            if (G_itemToSell->itemdesc.type == EQUIP_OBJECT_TYPE_QUIVER)
            {
                ...
                {
                    case BOLT_TYPE_NORMAL:strcpy (stmp2, "normal");
                        break;
                    case BOLT_TYPE_POISON:strcpy (stmp2, "green");
                        break;
                    case BOLT_TYPE_PIERCING:strcpy (stmp2, "brown");
                        break;
                    case BOLT_TYPE_FIRE:strcpy (stmp2, "red");
                        break;
                    case BOLT_TYPE_ELECTRICITY:strcpy (stmp2, "yellow");
                        break;
                    case BOLT_TYPE_MANA_DRAIN:strcpy (stmp2, "purple");
                        break;
                    case BOLT_TYPE_ACID:strcpy (stmp2, "white");
                        break;
                    default:DebugCheck (0); /* bad bolt! */
                        break;
                }
                MessagePrintf("^009You bought 12 %s arrows for %s", stmp2, stmp);
            }
            else
                MessagePrintf("^009Sold!! for %s", stmp);
...
T_void
StoreRequestBuyItem(T_inventoryItemStruct *p_item)
{
...
T_void
StoreSetControlCallback(T_storeCallback callback)
{
...
E_Boolean
StoreWillBuy(E_equipObjectTypes type)
{
...
T_void
StoreAddBuyType(E_equipObjectTypes type)
{
...
StoreUIStart(T_word32 formNum)
{
    ...
    switch (adv)
    {
        ...
        case ADVENTURE_UNKNOWN:DebugCheck(0);
            break;
    }
...
T_void
StoreConvertCurrencyToString(T_byte8 *tempString, T_word32 amount)
{
...
T_word16
StoreGetBuyValue(T_inventoryItemStruct *p_inv)
{
    T_word16 buyValue = 0;
    T_word16 objtype;
