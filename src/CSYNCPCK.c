...
static T_void
ClientSyncUpdateReceived(T_void);
/* stats */
...
/* Internal prototypes: */
static T_void
IClientSyncDestroyPacketHistory(T_void);
static T_void
IClientSyncAddPacketToHistory(T_packetLong *p_packet);
...
static E_Boolean
ICheckSyncIdHistory(T_word16 nextId, T_word16 syncNum);
static T_void
ICheckCollides(T_void);
...
T_void
ClientSyncInitPlayersHere(T_void)
{
    T_word16 i;
 ...
 *<!-----------------------------------------------------------------------*/
T_void
ClientSyncPacketEvaluate(T_syncronizePacket *p_sync)
{
    T_3dObject *p_playerObj;
...
//printf("@@p:%d s:%d\n", ObjectGetServerId(p_playerObj), ObjectGetStance(p_playerObj)) ;
            ...
            SyncMemAdd("  vel %d %d %d\n",
                       ObjectGetXVel(p_playerObj),
                       ObjectGetYVel(p_playerObj),
                       ObjectGetZVel(p_playerObj));
            DebugCheck(playerNum < MAX_SYNC_PLAYERS);
...
//                        MessageAdd("Item taken.") ;
        ...
        case PLAYER_ACTION_SYNC_COMPLETE:
            /* Not currently implemented. */
            break;
        case PLAYER_ACTION_GOTO_PLACE:ClientSetNextPlace(p_actionData[0], 0);
            break;
        case PLAYER_ACTION_DROP_AT:
            ...
            break;
        case PLAYER_ACTION_PAUSE_GAME_TOGGLE:ClientTogglePause();
            break;
...
//                StatsSaveCharacter(StatsGetActive()) ;
            ...
            break;
        case PLAYER_ACTION_ID_SELF: p_playerID = IFindByName((T_byte8 *) p_actionData);
...
T_void
ClientSyncSendActionActivateForward(T_void)
{
...
T_void
ClientSyncSendActionLeaveLevel(
    T_word16 newLevel)
{
...
T_void
ClientSyncSendActionPickUpItem(
    T_word16 itemID,
    E_Boolean autoStore)
{
...
T_void
ClientSyncSendGotoPlace(T_word16 newPlace)
{
...
T_void
ClientSyncSendActionPauseGameToggle(T_void)
{
...
T_void
ClientSyncSendActionAbortLevel(T_void)
{
...
T_void
ClientSyncPacketProcess(T_syncronizePacket *p_sync)
{
    T_word16 player;
...
/* LES: 06/17/06  Created */
T_void
ClientSyncReceiveRetransmitPacket(
...
static T_void
IAddToSyncObjectIdHistory(T_word16 nextId, T_word16 syncNum)
{
...
T_byte8
ClientSyncGetNumberPlayers(T_void)
{
    return (T_byte8) G_numPlayers;
}

T_objMoveStruct *
ClientSyncGetLastGoodMove(T_void)
{
    return &G_lastGoodObjMove;
}

T_void
ClientSyncReset(T_void)
{
    G_syncAhead = 0;
    G_lastTimeSyncSent = 0;
}
...
T_gameGroupID
ClientSyncGetGameGroupID(T_void)
{
    return G_groupID;
}

T_void
ClientSyncSetGameGroupID(T_gameGroupID groupID)
{
...
T_void
ClientSyncSendIdSelf(T_byte8 *p_name)
{
    T_word16 i;
    T_word16 len;
    T_byte8 buffer[40];
