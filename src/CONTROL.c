...
static T_3dObject *
ControlLookAt(T_word16 x, T_word16 y);
static T_void
ControlExamineObject(T_3dObject *p_obj);
static T_word16
ControlSetMovePointer(T_word16 mx, T_word16 my);
 ...
 *--------------------------------------------------------------------------*/
static T_void
ControlMouseControlForUI(
    E_mouseEvent event,
    T_word16 x,
    T_word16 y,
    T_buttonClick button);
static T_void
ControlMouseControlForGame(
    E_mouseEvent event,
    T_word16 x,
    T_word16 y,
    T_buttonClick button);
...
#   endif
    ...
    if (!EscapeMenuIsOpen())
    {
        ...
        switch (event)
        {
            ...
            case MOUSE_EVENT_START:G_resetNeeded = FALSE;
            ...
            case MOUSE_EVENT_MOVE:
                ...
                switch (G_mouse.mode)
                {
                    case CONTROL_MOUSE_MODE_NORMAL:G_lookAngle = 0;
                        G_lookOffset = 0;
                        /* do nothing special */
                        if (ButtonIsMouseOverButton() == TRUE)
                        {
                    ...
                    case CONTROL_MOUSE_MODE_MOVE:G_lookAngle = 0;
                        G_lookOffset = 0;
                        jumpTimer = JUMP_RESET_TIME;
...
ControlSetDefaultPointer(E_controlMousePointerType type)
{
    ...
    if (G_mouse.type == type)
    {
        ...
        switch (type)
        {
            case CONTROL_MOUSE_POINTER_DEFAULT:strcpy(stmp, "UI/MOUSE/DEFAULT");
                hotx = 0;
                hoty = 0;
                break;

            case CONTROL_MOUSE_POINTER_LOOK:strcpy(stmp, "UI/MOUSE/LOOK");
                hotx = 6;
                hoty = 3;
                break;

            case CONTROL_MOUSE_POINTER_NOLOOK:strcpy(stmp, "UI/MOUSE/NOLOOK");
                hotx = 6;
                hoty = 3;
                break;

            case CONTROL_MOUSE_POINTER_MOVE_N:hotx = 6;
                hoty = 0;
                sprintf(stmp, "UI/MOUSE/DIR%1.1d", type - 3);
                break;

            case CONTROL_MOUSE_POINTER_MOVE_NE:hotx = 15;
                hoty = 0;
                sprintf(stmp, "UI/MOUSE/DIR%1.1d", type - 3);
                break;

            case CONTROL_MOUSE_POINTER_MOVE_E:hotx = 18;
                hoty = 3;
                sprintf(stmp, "UI/MOUSE/DIR%1.1d", type - 3);
                break;

            case CONTROL_MOUSE_POINTER_MOVE_SE:hotx = 15;
                hoty = 7;
                sprintf(stmp, "UI/MOUSE/DIR%1.1d", type - 3);
                break;

            case CONTROL_MOUSE_POINTER_MOVE_S:hotx = 6;
                hoty = 10;
                sprintf(stmp, "UI/MOUSE/DIR%1.1d", type - 3);
                break;

            case CONTROL_MOUSE_POINTER_MOVE_SW:hotx = 0;
                hoty = 7;
                sprintf(stmp, "UI/MOUSE/DIR%1.1d", type - 3);
                break;

            case CONTROL_MOUSE_POINTER_MOVE_W:hotx = 0;
                hoty = 3;
                sprintf(stmp, "UI/MOUSE/DIR%1.1d", type - 3);
                break;

            case CONTROL_MOUSE_POINTER_MOVE_NW:hotx = 0;
                hoty = 0;
                sprintf(stmp, "UI/MOUSE/DIR%1.1d", type - 3);
                break;

            default:DebugCheck(-1); /* shouldn't get here */
        }
