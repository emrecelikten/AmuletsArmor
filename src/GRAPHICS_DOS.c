 ...
 *
 * TODO: Remove Win32 stuff and factor out common functions.
 *
 * @addtogroup GRAPHICS
...
static T_void
IInstallVerticalInterrupt(T_void);
static T_void
IUninstallVerticalInterrupt(T_void);
#if defined(WATCOM)
static T_void __interrupt __far IVerticalRetrace(T_void) ;
#endif

static T_void
ICheckPaletteChange(T_void);
static T_void
ITransferPalette(T_void);
static T_void
IConfirmPaletteChange(T_void);
 ...
 *<!-----------------------------------------------------------------------*/
T_void
GrDrawPixel(T_word16 x, T_word16 y, T_color color)
{
...
T_void
GrDrawTranslucentPixel(T_word16 x, T_word16 y, T_color color)
{
    T_byte8 *p_destination;
 ...
 *<!-----------------------------------------------------------------------*/
T_void
GrSetBitFont(T_bitfont *p_bitfont)
{
 ...
 *<!-----------------------------------------------------------------------*/
T_void
GrSetCursorPosition(T_word16 x_position, T_word16 y_position)
{
 ...
 *<!-----------------------------------------------------------------------*/
T_void
GrDrawCharacter(T_byte8 character, T_color color)
{
    T_word16 height;
    T_word16 width;
    T_byte8 *p_line;
 ...
 *<!-----------------------------------------------------------------------*/
T_void
GrDrawText(T_byte8 *text, T_color color)
{
 ...
 *<!-----------------------------------------------------------------------*/
T_void
GrDrawShadowedText(T_byte8 *text, T_color color, T_color shadow)
{
    T_word16 xPos;
    T_word16 yPos;
    T_byte8 *textPtr;
 ...
 *<!-----------------------------------------------------------------------*/
T_word16
GrGetCharacterWidth(T_byte8 character)
{
    T_word16 width;
...
T_void
GrInvertHorizontalLine(
    T_word16 x1,
    T_word16 y1,
    T_word16 x2);

T_void
GrInvertVerticalLine(
    T_word16 x1,
    T_word16 y1,
    T_word16 y2);
...
T_void
GrSetWorkingPage(T_word16 page)
{
...
T_byte8 *
GrGetWorkingPage(T_void)
{
    return G_workingStart;
}

T_void
GrSetVisualPage(T_word16 page)
{
    DebugRoutine("GrSetWorkingPage");
    DebugCheck(page < MAX_PAGES);
#ifdef DOS32
...
T_void
GrSelectPlanes(T_word16 planes)
{
#ifdef DOS32
    planes <<= 8 ;
    planes |= 2 ;
 ...
 *<!-----------------------------------------------------------------------*/
T_screen
GrScreenAllocPartial(T_word16 ySize)
{
    T_screen screen;
