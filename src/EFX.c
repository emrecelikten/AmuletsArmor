...
static T_void
EfxCreateBloodSplat(T_efxID myID, E_Boolean translucent);
static T_void
EfxUpdateBloodSplat(T_efxID myID);
static T_void
EfxAddObjectToList(T_doubleLinkList list, T_3dObject *object);
static T_void
EfxCreateGenericShrapnel(T_word16 objectType,
                         T_efxID myID,
                         E_Boolean addToList,
                         E_Boolean ignoreGravity);
static T_void
EfxCreateGenericExplosion(T_word16 objectType,
                          T_efxID myID,
                          E_Boolean addToList,
                          T_sword16 zPixelOffset);

static T_void
EfxUpdateGeneric(T_efxID myID);
    ...
    if (type < EFX_UNKNOWN)
    {
        ...
        if (p_efx != NULL)
        {
            ...
            if (p_efx->objectList != DOUBLE_LINK_LIST_BAD)
            {
                ...
                switch (type)
                {
                    case EFX_BLOOD_SHRAPNEL:EfxCreateBloodSplat(p_efx->myID, transparent);
                        break;
                    ...
                    case EFX_TELEPORT:p_efx->duration = 100;
                    ...
                    case EFX_FIRE_SHRAPNEL:p_efx->duration = 150;
                    ...
                    case EFX_FLAME_SHRAPNEL:p_efx->duration = 80;
                    ...
                    case EFX_BONE_SHRAPNEL:p_efx->duration = 80;
                    ...
                    case EFX_ELECTRIC_SHRAPNEL:p_efx->duration = 60;
                    ...
                    case EFX_MAGIC_SHRAPNEL:p_efx->duration = 60;
...
EfxCreateBloodSplat(T_efxID myID, E_Boolean translucent)
{
    ...
    for (i = 0; i < p_efx->numberOf; i++)
    {
        ...
        for (j = 0; j < numSplatsInGroup; j++)
        {
            ...
            if (translucent == TRUE)
                ObjectMakeTranslucent(p_splooch);
            ObjectAdd(p_splooch);
