...
T_void
ClientUpdateHealth(T_void);

T_word16
ClientGetDelta(T_void);

T_void
ClientGotoForm(T_word32 formNumber);

static T_void
ClientOverlayDone(T_word16 animnum, T_byte8 flag);
...
static T_void
IClientAttemptOpeningForwardWall(T_void);

#ifndef COMPILE_OPTION_FRAME_SPEED_OFF
static T_void
IClientUpdateAndDrawFramesPerSecond(
    T_word16 left,
    T_word16 bottom);
#else
...
static void
IDrawCrosshairs(T_word16 left, T_word16 top, T_word16 right, T_word16 bottom)
{
    /* Find center x and y */
...
static T_3dObject *
IDrawTargetItem(T_word16 x, T_word16 y, T_word16 drawX, T_word16 drawY)
{
    ...
    T_word16 bottom)
{
    ...
    if ((InventoryWeaponIsBow()) && (!isGrabDrawn))
    {
        ...
        if (ammotype == BOLT_TYPE_UNKNOWN)
            ammotype = BOLT_TYPE_NORMAL;

        switch (ammotype)
        {
            case BOLT_TYPE_NORMAL:drawcolor = 211;
                break;

            case BOLT_TYPE_POISON:drawcolor = 128;
                break;

            case BOLT_TYPE_PIERCING:drawcolor = 31;
                break;

            case BOLT_TYPE_FIRE:drawcolor = 144;
                break;

            case BOLT_TYPE_ELECTRICITY:drawcolor = 160;
                break;

            case BOLT_TYPE_MANA_DRAIN:drawcolor = 112;
                break;

            case BOLT_TYPE_ACID:drawcolor = 83;
                break;
...
static T_void
IClientDrawStatus(T_word16 left, T_word16 bottom)
{
                        ...
                        if (!ClientIsDead())
                        {
                            ...
                            if (timeHeld)
                            {
                                ...
                                PlayerAccelDirection(
                                    playerMoveAngle + INT_ANGLE_90, timeHeld);
                            }
...
void
IClientHandleMapKeys(T_word16 scankey)
{
    T_sword32 zoom;
...
break;
                ...
                case KEYBOARD_EVENT_BUFFERED:break;
                ...
                case KEYBOARD_EVENT_RELEASE:break;
...
E_Boolean
ClientDropInventoryItem(T_word16 numItems, T_word16 itemType, T_word16 accData)
{
