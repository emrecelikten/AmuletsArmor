 ...
 *-------------------------------------------------------------------------*/
/**
 * Banner window for Notes.  Allows users to enter text that will be
 * saved with the character.
 *
 * @addtogroup NOTES
 * @brief Notes Banner Window
...
T_void
NotesChangeJournalPage(T_buttonID buttonID)
{
    T_sword16 delta;
...
T_void
NotesGotoPageID(T_word16 notePageID)
{
    T_word16 i;
    T_word16 notePage = 0;
...
T_void
NotesTransferJournalPage(T_void)
{
    T_word16 pageNum;
    T_word16 pageID;
...
E_Boolean
NotesJournalWindowIsAt(T_word16 x, T_word16 y)
{
