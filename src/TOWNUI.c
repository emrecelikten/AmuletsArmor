...
static T_void
TownUIUpdateGraphics(T_void);
static T_void
TownUIGotoPlace(T_buttonID buttonID);
static T_void
TownRedrawChatList(T_void);
static T_void
TownUISendChatMessage(T_TxtboxID TxtboxID);
static T_void
TownUIUpdateQuestInfo(T_void);
static T_void
TownUINextQuest(T_buttonID buttonID);
static T_void
TownUILastQuest(T_buttonID buttonID);
static T_void
TownUIUserBoxUp(T_buttonID buttonID);
static T_void
TownUIUserBoxDn(T_buttonID buttonID);
static T_void
TownUIDescriptionBoxUp(T_buttonID buttonID);
static T_void
TownUIDescriptionBoxDn(T_buttonID buttonID);
...
//    KeyboardSetEventHandler (TownUIHandleKey);
    ...
    {
        iSucceeded = TownUIFinishedQuest(LEVEL_STATUS_STARTED,
                                         (T_byte8) PeopleHereGetNumInGroupGame(),
                                         StatsGetCurrentQuestNumber());//for multiplayer, assume we haven't won
...
E_Boolean
TownPersonInChat(T_byte8 *personName)
{
...
T_void
TownAddPerson(T_byte8 *personName)
{
    T_byte8 *data;
    T_word32 size;
    T_word32 tocpy;
...
T_void
TownRemovePerson(T_byte8 *personName)
{
    T_doubleLinkListElement element, nextElement;
    T_byte8 *data;
    T_word32 tocmp;
...
T_void
TownUIAddMessage(T_byte8 *playerName, T_byte8 *message)
{
    T_byte8 *text;
    T_word16 i;
    T_word16 size = 0;
...
T_void
TownUISetAdventureCompleted(T_void)
{
...
E_Boolean
TownUIFinishedQuest(T_word16 multiplayerStatus, T_byte8 numPlayers, T_word16 currentQuest)
{
...
E_Boolean
TownUICompletedMapLevel(T_word16 mapLevel)
{
    T_iniFile idata;
    T_word16 currentQuest = 0;
    T_byte8 stmp[8192];
