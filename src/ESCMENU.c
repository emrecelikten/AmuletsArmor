 ...
 *--------------------------------------------------------------------------*/
void
EscapeMenuEnterState(T_byte8 aState);
void
EscapeMenuExitState(void);
void
EscapeMenuGotoState(T_byte8 aState);
static const T_byte8 *
EscMenuKeyToString(T_byte8 scankey);
static void
EscapeMenuSetOptions(void);
...
void
EscapeMenuKeyHandler(E_keyboardEvent event, T_word16 scankey)
{
    T_byte8 i;
...
//    printf("ESC-Key: %d %d\n", event, scankey);
    ...
    switch (event)
    {
        case KEYBOARD_EVENT_RELEASE:
            ...
            break;
        case KEYBOARD_EVENT_PRESS:G_escKeyPressed = TRUE;
            if (G_escMenuAssignMode)
            {
                ...
                if (scankey != KEY_SCAN_CODE_ESC)
                {
                    ...
                    for (i = 0; i < sizeof(G_keys) / sizeof(G_keys[0]); i++)
                    {
                        if (G_keys[i] == scankey)
                        {
                            if ((strstr(G_keyboardMapping[i], "ALT+") == 0)
                                == (strstr(G_keyboardMapping[G_escMenuAssignKeyMap], "ALT+") == 0))
                            {
                                G_keys[i] = 0;
                            }
                        }
...
static void
EscapeMenuSetOptionButtonText(T_byte8 aOptionIndex, const char *aText, T_byte8 aColor)
{
    T_buttonID buttonID;
