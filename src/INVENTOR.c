...
/* internal routines */
static E_Boolean
InventoryDestroyElement(T_doubleLinkListElement killme);
static T_doubleLinkListElement
InventoryIsItemInArea(E_inventoryType where, T_byte8 gx1, T_byte8 gx2, T_byte8 gy1, T_byte8 gy2, T_byte8 page);
static E_Boolean
InventoryDrawElement(T_doubleLinkListElement element);
static E_Boolean
InventoryFindOverlap(T_doubleLinkListElement element);
static T_void
InventorySetPlayerColor(E_equipLocations location);
static E_Boolean
InventoryDoItemEffect(T_inventoryItemStruct *p_inv, E_effectTriggerType trigger);

static T_void
InventorySortByType(E_inventoryType which);
static T_void
InventoryGroupItems(E_inventoryType whichInv);
static T_void
InventoryClearCurrentLocations(E_inventoryType which);
static T_void
InventorySetWindowSizeDefaults(T_void);
static T_doubleLinkListElement
InventoryFindElement(E_inventoryType whichinv,
                     T_inventoryItemStruct *p_inv);
static T_void
InventoryUpdateStoreItemTitlePage(T_void);
static T_void
InventorySelectNextStoreInventoryPage(T_buttonID buttonID);
static T_void
InventorySelectLastStoreInventoryPage(T_buttonID buttonID);
...
InventoryDestroyElement(T_doubleLinkListElement killme)
{
    ...
    {
        if (G_inventoryLocations[i] == killme)
            G_inventoryLocations[i] = DOUBLE_LINK_LIST_ELEMENT_BAD;
    }
...
T_void
InventoryFindInventoryItemPage(E_inventoryType which, T_inventoryItemStruct *thisitem)
{
    T_inventoryItemStruct *p_inv;
    T_doubleLinkListElement element;
...
InventoryInventoryWindowIsAt(T_word16 locx, T_word16 locy)
{
    ...
    if (InventoryFindInventoryWindow(locx, locy) != INVENTORY_UNKNOWN)
        retvalue = TRUE;
...
InventoryFindInventoryWindow(T_word16 locx, T_word16 locy)
{
    ...
    if (i == INVENTORY_PLAYER)
    {
        ...
        if (BannerFormIsOpen(BANNER_FORM_INVENTORY) != TRUE)
            retvalue = INVENTORY_UNKNOWN;
    }
...
InventoryEquipmentWindowIsAt(T_word16 locx, T_word16 locy)
{
    ...
    if (locx >= G_equipmentWindowX1 &&
        ...
        BannerFormIsOpen(BANNER_FORM_EQUIPMENT))
        retvalue = TRUE;
...
//                        }
        ...
        if (p_inv->itemdesc.objectDestroyOn == EFFECT_TRIGGER_USE)
        {
            ...
            if (p_inv->numitems > 1)
            {
                ...
                InventoryDestroyElement(p_inv->elementID);
...
InventoryCheckItemInReadyHand(T_void)
{
    ...
    if (G_inventoryLocations[EQUIP_LOCATION_READY_HAND] != DOUBLE_LINK_LIST_ELEMENT_BAD)
    {
        /*yes, return pointer to T_3dobject for it */
        retvalue =
            (T_inventoryItemStruct *) DoubleLinkListElementGetData(G_inventoryLocations[EQUIP_LOCATION_READY_HAND]);
    }
...
T_inventoryItemStruct *
InventoryTakeObject(E_inventoryType which, T_3dObject *item)
{
    T_word16 objtype;
    T_byte8 *desc1;
    T_resource res;
    ...
    else
    {
        ...
        if (PictureExist(stmp))
        {
            ...
            else
            {
                ...
                if (InventoryIsUseableByClass(p_inv) == FALSE)
                    destroyme = TRUE;
                    ...
                    p_inv->itemdesc.subtype == THING_TYPE_RUNE)
                {
                    ...
                    {
                        if (ObjectGetType(p_inv->object) < 309)
                            destroyme = TRUE;
                    }
                    else if (spellsys == SPELL_SYSTEM_MAGE)
                    {
                        if (ObjectGetType(p_inv->object) > 308)
                            destroyme = TRUE;
                    }
                    else
                    {
                        if (ObjectGetType(p_inv->object) > 303 &&
                            ObjectGetType(p_inv->object) < 309)
                            destroyme = TRUE;
                        if (ObjectGetType(p_inv->object) > 313)
                            destroyme = TRUE;
                    }
                }
                     ...
                     i--)
                {
                         ...
                         j--)
                    {
                        ...
                        if (temp == NULL)
                        {
                            ...
                            p_inv->locx =
                                p_inv->gridstartx * INVENTORY_GRID_WIDTH
                                    + G_inventories[INVENTORY_PLAYER].locx1 + 1;
                            p_inv->locy =
                                p_inv->gridstarty * INVENTORY_GRID_HEIGHT
                                    + G_inventories[INVENTORY_PLAYER].locy1 + 1;
                    ...
                    if (InventoryCheckClassCanUseArmor(p_inv, FALSE) == TRUE)
                    {
                        ...
                        switch (p_inv->itemdesc.subtype)
                        {
                            ...
                            case EQUIP_ARMOR_TYPE_BRACING_PLATE:
                                if (G_inventoryLocations[EQUIP_LOCATION_LEFT_ARM]
                                ...
                                else if (G_inventoryLocations[EQUIP_LOCATION_RIGHT_ARM]
                                    == DOUBLE_LINK_LIST_ELEMENT_BAD)
                                {
                            ...
                            case EQUIP_ARMOR_TYPE_LEGGINGS_PLATE:
                                if (G_inventoryLocations[EQUIP_LOCATION_LEGS]
                                    == DOUBLE_LINK_LIST_ELEMENT_BAD)
                                {
                            ...
                            case EQUIP_ARMOR_TYPE_HELMET_PLATE:
                                if (G_inventoryLocations[EQUIP_LOCATION_HEAD]
                                    == DOUBLE_LINK_LIST_ELEMENT_BAD)
                                {
                            ...
                            case EQUIP_ARMOR_TYPE_BREASTPLATE_PLATE:
                                if (G_inventoryLocations[EQUIP_LOCATION_CHEST]
                                    == DOUBLE_LINK_LIST_ELEMENT_BAD)
                                {
...
InventoryThrowObjectIntoWorld(T_word16 x, T_word16 y)
{
    ...
    if (InventoryObjectIsInMouseHand())
    {
        ...
        {
            p_inv =
                (T_inventoryItemStruct *) DoubleLinkListElementGetData(G_inventoryLocations[EQUIP_LOCATION_MOUSE_HAND]);
    ...
    if (thisInv != INVENTORY_UNKNOWN)
    {
        ...
        else
        {
            ...
            if (InventoryObjectIsInMouseHand() == TRUE)
            {
                    ...
                    p_inv->gridstarty + p_inv->gridspacesy <= G_inventories[thisInv].gridspacesy)
                {
                    ...
                    if (temp == DOUBLE_LINK_LIST_ELEMENT_BAD)
                    {
                        ...
                        p_inv->locx =
                            p_inv->gridstartx * INVENTORY_GRID_WIDTH + G_inventories[thisInv].locx1
                                + 1;
                        p_inv->locy =
                            p_inv->gridstarty * INVENTORY_GRID_HEIGHT + G_inventories[thisInv].locy1
                                + 1;
            ...
            else
            {
                ...
                temp = InventoryIsItemInArea(thisInv, gsx,
                                             gsx + 1, gsy, gsy + 1, G_inventories[thisInv].curpage);
                if (temp != DOUBLE_LINK_LIST_ELEMENT_BAD)
                {
                    /* found something */
            ...
            if (InventoryObjectIsInMouseHand() == TRUE)
            {
                ...
                if (p_inv->itemdesc.type == EQUIP_OBJECT_TYPE_ARMOR)
                {
                    switch (p_inv->itemdesc.subtype)
                    {
                        ...
                        case EQUIP_ARMOR_TYPE_LEGGINGS_CLOTH:
                        case EQUIP_ARMOR_TYPE_LEGGINGS_CHAIN:
                        case EQUIP_ARMOR_TYPE_LEGGINGS_PLATE:location = EQUIP_LOCATION_LEGS;
                            break;

                        case EQUIP_ARMOR_TYPE_HELMET_CLOTH:
                        case EQUIP_ARMOR_TYPE_HELMET_CHAIN:
                        case EQUIP_ARMOR_TYPE_HELMET_PLATE:location = EQUIP_LOCATION_HEAD;
                            break;

                        case EQUIP_ARMOR_TYPE_BREASTPLATE_CLOTH:
                        case EQUIP_ARMOR_TYPE_BREASTPLATE_CHAIN:
                        case EQUIP_ARMOR_TYPE_BREASTPLATE_PLATE:location = EQUIP_LOCATION_CHEST;
                            break;

                        default:DebugCheck(0);
                            break;
                    }
                    ...
                    for (i = EQUIP_LOCATION_NECK; i <= EQUIP_LOCATION_RING_4; i++)
                    {
                            ...
                            y <= G_inventoryBoxCoordinates[i][EQUIP_LOC_Y2])
                        {
                            ...
                            if (location == EQUIP_LOCATION_NECK &&
                                ...
                                p_inv->itemdesc.type == EQUIP_OBJECT_TYPE_RING)
                                canstore = TRUE;
                            break;
                        }
                    }
    ...
    for (i = 0; i < G_inventories[which].gridspacesx; i++)
    {
        for (j = 0; j < G_inventories[which].gridspacesy; j++)
        {
            ...
            GrDrawRectangle(G_inventories[which].locx1 + (i * (INVENTORY_GRID_WIDTH)),
                            ...
                            G_inventories[which].locy1
                                + ((j + 1) * (INVENTORY_GRID_HEIGHT)), INVENTORY_BASE_COLOR);
            GrDrawFrame(G_inventories[which].locx1 + (i * (INVENTORY_GRID_WIDTH)),
                        G_inventories[which].locy1 + (j * (INVENTORY_GRID_HEIGHT)),
                        G_inventories[which].locx1 + ((i + 1) * (INVENTORY_GRID_WIDTH)),
                        G_inventories[which].locy1
                            + ((j + 1) * (INVENTORY_GRID_HEIGHT)), INVENTORY_GRID_COLOR);
        }
    }
    ...
    if (InventoryObjectIsInReadyHand())
    {
        ...
        switch (p_inv->itemdesc.type)
        {
            case EQUIP_OBJECT_TYPE_WEAPON:
                switch (p_inv->itemdesc.subtype)
                {
                    case EQUIP_WEAPON_TYPE_AXE:OverlaySetAnimation(ANIMATION_OVERLAY_AXE);
                        break;
                    case EQUIP_WEAPON_TYPE_DAGGER:OverlaySetAnimation(ANIMATION_OVERLAY_DAGGER);
                        break;
                    case EQUIP_WEAPON_TYPE_LONGSWORD:OverlaySetAnimation(ANIMATION_OVERLAY_SWORD);
                        break;
                    case EQUIP_WEAPON_TYPE_SHORTSWORD:OverlaySetAnimation(ANIMATION_OVERLAY_SWORD);
                        break;
                    case EQUIP_WEAPON_TYPE_MACE:OverlaySetAnimation(ANIMATION_OVERLAY_MACE);
                        break;
                    case EQUIP_WEAPON_TYPE_STAFF:OverlaySetAnimation(ANIMATION_OVERLAY_STAFF);
                        break;
                    case EQUIP_WEAPON_TYPE_CROSSBOW:OverlaySetAnimation(ANIMATION_OVERLAY_XBOW);
                        break;
                    case EQUIP_WEAPON_TYPE_WAND:OverlaySetAnimation(ANIMATION_OVERLAY_NULL);
                        break;
                    default:
            ...
            case EQUIP_OBJECT_TYPE_WAND:OverlaySetAnimation(ANIMATION_OVERLAY_NULL);
                break;
            ...
            case EQUIP_OBJECT_TYPE_QUIVER:OverlaySetAnimation(ANIMATION_OVERLAY_NULL);
                break;
...
static T_doubleLinkListElement
InventoryIsItemInArea(E_inventoryType invWin, T_byte8 gx1, T_byte8 gx2, T_byte8 gy1, T_byte8 gy2, T_byte8 page)
{
...
InventorySetPlayerColor(E_equipLocations location)
{
    ...
    switch (location)
    {
        case EQUIP_LOCATION_HEAD:PlayerSetBodyPart(BODY_PART_LOCATION_HEAD, bodyPart);
            break;
        case EQUIP_LOCATION_RIGHT_ARM:PlayerSetBodyPart(BODY_PART_LOCATION_LEFT_ARM, bodyPart);
            break;
        case EQUIP_LOCATION_LEFT_ARM:PlayerSetBodyPart(BODY_PART_LOCATION_RIGHT_ARM, bodyPart);
            break;
        case EQUIP_LOCATION_CHEST:PlayerSetBodyPart(BODY_PART_LOCATION_CHEST, bodyPart);
            break;
        case EQUIP_LOCATION_LEGS:PlayerSetBodyPart(BODY_PART_LOCATION_LEGS, bodyPart);
            break;
        case EQUIP_LOCATION_SHIELD_HAND:PlayerSetBodyPart(BODY_PART_LOCATION_SHIELD, bodyPart);
            break;
        case EQUIP_LOCATION_READY_HAND:PlayerSetBodyPart(BODY_PART_LOCATION_WEAPON, bodyPart);
            break;
    }
...
E_Boolean
InventoryDoEffect(E_effectTriggerType trigger, E_equipLocations location)
{
    E_Boolean retvalue = FALSE;
    T_inventoryItemStruct *p_inv;
    T_word16 i;
...
E_Boolean
InventoryDestroyOn(E_effectTriggerType trigger, E_equipLocations location)
{
    E_Boolean retvalue = FALSE;
    T_inventoryItemStruct *p_inv;
...
E_Boolean
InventoryShouldDestroyOn(E_effectTriggerType trigger, E_equipLocations location)
{
    E_Boolean retvalue = FALSE;
    T_inventoryItemStruct *p_inv;
...
//    printf ("in destroy random stored\n");
    ...
    while (element != DOUBLE_LINK_LIST_ELEMENT_BAD)
    {
        ...
        if (p_inv->itemdesc.type == EQUIP_OBJECT_TYPE_THING &&
            (p_inv->itemdesc.subtype == THING_TYPE_RUNE ||
                p_inv->itemdesc.subtype == THING_TYPE_KEY))
            success = FALSE;
...
*/
            ...
            switch (p_inv->itemdesc.subtype)
            {
                ...
                case EQUIP_WEAPON_TYPE_DAGGER:SoundPlayByNumber(SOUND_SWING_SET3 + randSound, 200);
                    break;

                case EQUIP_WEAPON_TYPE_AXE:
                case EQUIP_WEAPON_TYPE_LONGSWORD:SoundPlayByNumber(SOUND_SWING_SET2 + randSound, 200);
                    break;

                case EQUIP_WEAPON_TYPE_SHORTSWORD:
                case EQUIP_WEAPON_TYPE_MACE:
                case EQUIP_WEAPON_TYPE_STAFF:SoundPlayByNumber(SOUND_SWING_SET1 + randSound, 200);
                    break;

                case EQUIP_WEAPON_TYPE_CROSSBOW:SoundPlayByNumber(SOUND_XBOW, 255);
                    break;

                case EQUIP_WEAPON_TYPE_WAND:SoundPlayByNumber(SOUND_ELECTRIC1, 255);
                    break;
    ...
    if (InventoryObjectIsInReadyHand())
    {
        ...
        if (p_inv->itemdesc.type == EQUIP_OBJECT_TYPE_WEAPON)
        {
            ...
            switch (p_inv->itemdesc.subtype)
            {
                ...
                case EQUIP_WEAPON_TYPE_DAGGER:SoundPlayByNumber(SOUND_DAGGERHIT + randSound, 255);
                    break;

                case EQUIP_WEAPON_TYPE_STAFF:
                case EQUIP_WEAPON_TYPE_MACE:SoundPlayByNumber(SOUND_MACEHIT + randSound, 255);
                    break;

                case EQUIP_WEAPON_TYPE_AXE:
                case EQUIP_WEAPON_TYPE_LONGSWORD:
                case EQUIP_WEAPON_TYPE_SHORTSWORD:SoundPlayByNumber(SOUND_SWORDHIT + randSound, 255);
                    break;

                default:SoundPlayByNumber(SOUND_FISTHIT + randSound, 255);
                    break;
            }
        }
    }
...
//    if (BannerFormIsOpen (BANNER_FORM_INVENTORY)) InventoryDrawInventoryWindow(which);
    ...
    if (which == INVENTORY_STORE)
    {
        ...
        if (G_inventories[which].curpage > curpage)
            G_inventories[which].curpage = curpage;
...
T_void
InventorySetInventoryWindowLocation(E_inventoryType type,
                                    T_word16 x1,
                                    T_word16 y1,
                                    T_word16 x2,
                                    T_word16 y2,
...
T_void
InventoryMoveInventoryWindowLocation(E_inventoryType type,
                                     T_word16 dx,
                                     T_word16 dy)
{
    T_doubleLinkListElement element;
...
InventoryTransferItemBetweenInventories(T_inventoryItemStruct *p_inv,
...
T_void
InventoryAddObjectToInventory(E_inventoryType which,
                              T_word16 objID,
                              T_byte8 numToAdd)
{
...
T_void
InventoryRemoveObjectFromInventory(E_inventoryType which,
                                   T_word16 objID,
                                   T_byte8 numToRemove)
{
...
InventoryUpdateStoreItemTitlePage(T_void)
{
    ...
    if (element != DOUBLE_LINK_LIST_ELEMENT_BAD)
    {
        ...
        switch (p_inv->itemdesc.type)
        {
            case EQUIP_OBJECT_TYPE_WEAPON:strcpy (stmp, "Weapons");
                break;
            case EQUIP_OBJECT_TYPE_ARMOR:strcpy (stmp, "Armor");
                break;
            case EQUIP_OBJECT_TYPE_AMULET:strcpy (stmp, "Amulets");
                break;
            case EQUIP_OBJECT_TYPE_RING:strcpy (stmp, "Rings");
                break;
            case EQUIP_OBJECT_TYPE_POTION:strcpy (stmp, "Potions");
                break;
            case EQUIP_OBJECT_TYPE_WAND:strcpy (stmp, "Wands");
                break;
            case EQUIP_OBJECT_TYPE_SCROLL:strcpy (stmp, "Scrolls");
                break;
            case EQUIP_OBJECT_TYPE_FOODSTUFF:strcpy (stmp, "Food Items");
                break;
            case EQUIP_OBJECT_TYPE_POWERUP:DebugCheck (0); /* fail! */
                break;
            case EQUIP_OBJECT_TYPE_THING:strcpy (stmp, "Assorted Items");
                break;
            case EQUIP_OBJECT_TYPE_COIN:DebugCheck (0); /* fail! */
                break;
            case EQUIP_OBJECT_TYPE_BOLT:strcpy (stmp, "Bolts");
                break;
            case EQUIP_OBJECT_TYPE_QUIVER:strcpy (stmp, "Arrows");
                break;
            case EQUIP_OBJECT_TYPE_UNKNOWN:DebugCheck (0); /* fail! */
                break;
            default:DebugCheck (0); /* fail! */
                break;
        }
...
E_Boolean
InventoryDestroySpecificItem(E_inventoryType whichInventory,
                             T_word16 objectType,
...
//      StatsAddPlayerNotePage(304); /* Earthsmite */
    ...
    for (i = 0; i <= EQUIP_LOCATION_LEGS; i++)
        InventorySetPlayerColor(i);
...
E_Boolean
InventoryCheckClassCanUseWeapon(T_inventoryItemStruct *p_inv,
                                E_Boolean showMessage)
{
    ...
    switch (p_inv->itemdesc.subtype)
    {
        case EQUIP_WEAPON_TYPE_DAGGER:
            ...
            {
                if (showMessage == TRUE)
                    MessageAdd("You aren't allowed to use daggers.");
                canUse = FALSE;
            }
            break;
        ...
        case EQUIP_WEAPON_TYPE_AXE:
            ...
            {
                if (showMessage == TRUE)
                    MessageAdd("^005You aren't proficient with axes.");
                canUse = FALSE;
            }
            break;
        ...
        case EQUIP_WEAPON_TYPE_LONGSWORD:
            ...
            {
                if (showMessage == TRUE)
                    MessageAdd("^005You aren't proficient with longswords.");
                canUse = FALSE;
            }
        ...
        case EQUIP_WEAPON_TYPE_SHORTSWORD:
            ...
            {
                if (showMessage == TRUE)
                    MessageAdd("^005You aren't proficient with swords.");
                canUse = FALSE;
            }
            break;
        ...
        case EQUIP_WEAPON_TYPE_MACE:
            ...
            {
                if (showMessage == TRUE)
                    MessageAdd("^005You aren't proficient with maces.");
                canUse = FALSE;
            }
            break;
        ...
        case EQUIP_WEAPON_TYPE_CROSSBOW:
            ...
            {
                if (showMessage == TRUE)
                    MessageAdd("^005You aren't proficient with bows.");
                canUse = FALSE;
            }
            break;
...
E_Boolean
InventoryCheckClassCanUseArmor(T_inventoryItemStruct *p_inv,
                               E_Boolean showMessage)
{
    E_Boolean canUse = TRUE;
    E_statsClassType ourclass = CLASS_UNKNOWN;
    ...
    if (p_inv->itemdesc.type == EQUIP_OBJECT_TYPE_ARMOR)
    {
        switch (p_inv->itemdesc.subtype)
        {
            ...
            case EQUIP_ARMOR_TYPE_BREASTPLATE_PLATE:
                ...
                {
                    if (showMessage == TRUE)
                        MessageAdd("^005You can't wear heavy armor");
                    canUse = FALSE;
                }
                break;
            ...
            case EQUIP_ARMOR_TYPE_BREASTPLATE_CHAIN:
                ...
                {
                    if (showMessage == TRUE)
                        MessageAdd("^005That armor is too heavy for you to wear.");
                    canUse = FALSE;
                }
                break;
