...
SpellsInitSpells(T_void)
{
    ...
    while (PictureExist(stmp))
    {
        ...
        {
            if (p_spell->code[i] == 1)
                p_spell->code[i] = KeyMap(KEYMAP_RUNE1);
            else if (p_spell->code[i] == 2)
                p_spell->code[i] = KeyMap(KEYMAP_RUNE2);
            else if (p_spell->code[i] == 3)
                p_spell->code[i] = KeyMap(KEYMAP_RUNE3);
            else if (p_spell->code[i] == 4)
                p_spell->code[i] = KeyMap(KEYMAP_RUNE4);
            else if (p_spell->code[i] == 5)
                p_spell->code[i] = KeyMap(KEYMAP_RUNE5);
            else if (p_spell->code[i] == 6)
                p_spell->code[i] = KeyMap(KEYMAP_RUNE6);
            else if (p_spell->code[i] == 7)
                p_spell->code[i] = KeyMap(KEYMAP_RUNE7);
            else if (p_spell->code[i] == 8)
                p_spell->code[i] = KeyMap(KEYMAP_RUNE8);
            else if (p_spell->code[i] == 9)
                p_spell->code[i] = KeyMap(KEYMAP_RUNE9);
        }
/*
        printf ("\n\nloading spell:\n");
        printf ("code = %d,%d,%d,%d\n",p_spell->code[0],
        ...
        switch (system)
        {
            case SPELL_SYSTEM_MAGE:
                if (p_spell->system == SPELL_SYSTEM_MAGE ||
                    ...
                    p_spell->system == SPELL_SYSTEM_ANY)
                    unload = FALSE;
                break;
...
T_byte8
SpellKeyToRuneID(T_byte8 scankey)
{
 ...
 *<!-----------------------------------------------------------------------*/
T_void
SpellsAddRune(T_buttonID buttonID)
{
 ...
 *<!-----------------------------------------------------------------------*/
T_void
SpellsClearRunes(T_buttonID buttonID)
{
    T_word16 i;
    T_bitmap *pic;
    T_resource res;
 ...
 *<!-----------------------------------------------------------------------*/
T_void
SpellsBackspace(T_buttonID buttonID)
{
    T_word16 i;
    T_bitmap *pic;
    T_resource res;
...
RuneIDToKey(T_byte8 aRuneID)
{
    ...
    switch (aRuneID)
    {
        case 1: key = KeyMap(KEYMAP_RUNE1);
            break;
        case 2: key = KeyMap(KEYMAP_RUNE2);
            break;
        case 3: key = KeyMap(KEYMAP_RUNE3);
            break;
        case 4: key = KeyMap(KEYMAP_RUNE4);
            break;
        case 5: key = KeyMap(KEYMAP_RUNE5);
            break;
        case 6: key = KeyMap(KEYMAP_RUNE6);
            break;
        case 7: key = KeyMap(KEYMAP_RUNE7);
            break;
        case 8: key = KeyMap(KEYMAP_RUNE8);
            break;
        case 9: key = KeyMap(KEYMAP_RUNE9);
            break;
        default: key = 0;
            break;
    }
...
T_byte8
RuneIDToOriginalScanKey(T_byte8 aRuneID)
{
    T_byte8 scankey = 0;
    ...
    switch (aRuneID)
    {
        case 1: scankey = KEY_SCAN_CODE_KEYPAD_1;
            break;
        case 2: scankey = KEY_SCAN_CODE_KEYPAD_2;
            break;
        case 3: scankey = KEY_SCAN_CODE_KEYPAD_3;
            break;
        case 4: scankey = KEY_SCAN_CODE_KEYPAD_4;
            break;
        case 5: scankey = KEY_SCAN_CODE_KEYPAD_5;
            break;
        case 6: scankey = KEY_SCAN_CODE_KEYPAD_6;
            break;
        case 7: scankey = KEY_SCAN_CODE_KEYPAD_7;
            break;
        case 8: scankey = KEY_SCAN_CODE_KEYPAD_8;
            break;
        case 9: scankey = KEY_SCAN_CODE_KEYPAD_9;
            break;
        default: scankey = 0;
            break;
    }
...
T_void
SpellsCastSpell(T_buttonID buttonID)
{
    E_Boolean success;
    ...
    else
    {
        ...
        while (element != DOUBLE_LINK_LIST_ELEMENT_BAD)
        {
            ...
            if (success == TRUE)
            {
                ...
                if (spellduration > MAX_EFFECT_DURATION)
                    spellduration = MAX_EFFECT_DURATION;

                /* figure power of spell */
                spellpower = p_spell->power + (p_spell->powermod * charlevel);
                if (spellpower > MAX_EFFECT_POWER)
                    spellpower = MAX_EFFECT_POWER;
                    ...
                    (EffectPlayerEffectIsActive(PLAYER_EFFECT_GOD_MODE) == TRUE))
                {
                    ...
                    if (success == TRUE)
                    {
                        ...
                        /* do a color effect */
                        ColorAddGlobal(
                            (T_sbyte8) p_spell->filtr >> 1,
                            (T_sbyte8) p_spell->filtg >> 1, (T_sbyte8) p_spell->filtb >> 1);
...
T_void
SpellsDrawInEffectRunes(T_word16 left,
                        T_word16 right,
                        T_word16 top,
                        T_word16 bottom)
{
...
T_void
SpellsSetRune(E_spellsRuneType type)
{
    DebugRoutine ("SpellsAddRune");
...
T_void
SpellsClearRune(E_spellsRuneType type)
{
