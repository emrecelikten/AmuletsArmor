...
/* Internal prototypes: */
static T_void
IAllocatePage(T_word16 num);
static T_void
IFreePage(T_word16 num);
static T_void
ICalculateUpperLeft(
    T_word16 left,
    T_word16 top,
    ...
    T_word16 *y);
static T_void
IDrawSinglePage(T_word16 page);
static T_void
IDrawWall(T_word16 numWall);
static T_void
IOverheadDisplay(T_word16 left, T_word16 top);
static T_void
ICompileView(T_void);
static T_void
IDrawLine(
 ...
 *<!-----------------------------------------------------------------------*/
T_void
OverheadSetSize(T_word16 sizeX, T_word16 sizeY)
{
 ...
 *<!-----------------------------------------------------------------------*/
T_void
OverheadSetOffset(T_word16 offsetX, T_word16 offsetY)
{
 ...
 *<!-----------------------------------------------------------------------*/
T_void
OverheadSetPosition(E_overheadPosition position)
{
 ...
 *<!-----------------------------------------------------------------------*/
T_void
OverheadSetFeatures(F_overheadFeature flags)
{
 ...
 *<!-----------------------------------------------------------------------*/
T_void
OverheadAddFeatures(F_overheadFeature flags)
{
    DebugRoutine("OverheadAddFeatures");
    DebugCheck(!(flags & OVERHEAD_FEATURE_UNKNOWN));
 ...
 *<!-----------------------------------------------------------------------*/
T_void
OverheadRemoveFeatures(F_overheadFeature flags)
{
 ...
 *<!-----------------------------------------------------------------------*/
T_void
OverheadSetCenterPoint(T_sword32 x, T_sword32 y)
{
 ...
 *<!-----------------------------------------------------------------------*/
T_void
OverheadSetZoomFactor(T_word32 zoom)
{
    ...
    T_word16 *y)
{
    ...
    switch (G_position)
    {
        case OVERHEAD_POSITION_UPPER_LEFT:xLeft = left + G_offsetX;
            yTop = top + G_offsetY;
            break;
        case OVERHEAD_POSITION_UPPER_RIGHT:xLeft = right - G_offsetX - G_sizeX;
            yTop = top + G_offsetY;
            break;
        case OVERHEAD_POSITION_LOWER_RIGHT:xLeft = right - G_offsetX - G_sizeX;
            yTop = bottom - G_offsetY - G_sizeY;
            break;
        case OVERHEAD_POSITION_LOWER_LEFT:xLeft = left + G_offsetX;
            yTop = bottom - G_offsetY - G_sizeY;
            break;
        case OVERHEAD_POSITION_CENTER:xLeft = left + ((right - left - G_sizeX) >> 1);
            yTop = top + ((bottom - top - G_sizeY) >> 1);
            break;
        case OVERHEAD_POSITION_BOTTOM_CENTER:xLeft = left + G_offsetX;
            yTop = bottom - G_offsetY - G_sizeY;
            break;
    }
