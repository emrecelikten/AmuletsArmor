...
static T_void
GuildUIUpdateGraphics(T_void);
static T_void
GuildUIBuildMapList(T_void);
static E_Boolean
GuildUIPlayerCanVisitMap(T_word16 which);
static T_void
GuildDisplayMapInfo(T_TxtboxID TxtboxID);
static T_void
GuildDisplayGameInfo(T_TxtboxID TxtboxID);
static T_void
GuildUIUpDisplay(T_buttonID buttonID);
static T_void
GuildUIDnDisplay(T_buttonID buttonID);
static T_void
GuildUIDrawGameList(T_void);
static T_void
GuildUIDrawPlayerList(T_void);
static T_void
GuildUICancelCreateGame(T_buttonID buttonID);
static T_void
GuildUIBeginGame(T_buttonID buttonID);
static T_void
GuildUIJoinGame(T_buttonID buttonID);
static T_void
GuildUICreateGame(T_buttonID buttonID);
static T_void
GuildUIRequestPlayerListForOpenGame(T_gameDescriptionStruct *p_game);
static T_void
GuildUIRequestGameList(T_void);
static T_void
GuildUIReset(T_void);
static T_mapDescriptionStruct *
GuildUIGetMapDescription(T_word16 mapIndex);
static T_word16
GuildUIGetSelectedAdventure(T_void);
...
GuildUIBuildMapList(T_void)
{
    ...
    while (PictureExist(stmp))
    {
        ...
        if (listdata != NULL)
            tempSize += strlen(listdata);
        temps = MemAlloc(tempSize + 2);
        if (listdata == NULL)
            sprintf (temps, "%s\r", stmp);
        else
            sprintf (temps, "%s%s\r", listdata, stmp);
        if (listdata != NULL)
            MemFree(listdata);
        listdata = temps;
...
GuildUIPlayerCanVisitMap(T_word16 which)
{
    ...
    while (element != DOUBLE_LINK_LIST_ELEMENT_BAD)
    {
        if (count == which)
        {
            ...
            if (p_mapDesc->mapKey == 0)
                canGo = TRUE;
            else if (StatsPlayerHasNotePage(p_mapDesc->mapKey))
                canGo = TRUE;
            break;
        }
        count++;
...
GuildUIUpDisplay(T_buttonID buttonID)
{
    ...
    if (ButtonGetData(buttonID) == GUILD_MAPS_LIST)
        GuildDisplayMapInfo(G_displayBoxes[GUILD_MAPS_LIST]);
    else if (ButtonGetData(buttonID) == GUILD_GAME_LIST)
...
GuildUIDrawGameList(T_void)
{
    ...
    while (element != DOUBLE_LINK_LIST_ELEMENT_BAD)
    {
        ...
        /* add to list */
        if (listData != NULL)
            size = strlen(listData);
        else
            size = 0;
        size += strlen(stmp);
        temp = MemAlloc(size + 2);
        if (listData != NULL)
            sprintf (temp, "%s%s", listData, stmp);
        else
            strcpy (temp, stmp);
        if (listData != NULL)
            MemFree(listData);
...
GuildUIDrawPlayerList(T_void)
{
    ...
    while (element != DOUBLE_LINK_LIST_ELEMENT_BAD)
    {
        ...
        temp = MemAlloc(size + 2);
        if (listData != NULL)
            sprintf (temp, "%s%s\r", listData, name);
        else
            sprintf (temp, "%s\r", name);
...
GuildUIClearPlayers(T_void)
{
    ...
    if (test != NULL)
        TxtboxSetData(G_displayBoxes[GUILD_GAME_DESC], "");
