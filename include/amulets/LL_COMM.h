...
CEXT int
ioOpenPort(T_word32 base, T_word32 irq);
CEXT int
ioClosePort(COMM Port);
...
/* I/O Routines 			****/
CEXT T_byte8
ioReadByte(int address);
CEXT int
ioWriteByte(int address, T_byte8 b);
CEXT int
ioReadPacket(COMM, PACKET);
CEXT int
ioWritePacket(COMM, PACKET);
...
ioGetHandShake(COMM);
CEXT void
ioSetHandShake(COMM, int);
CEXT int
ioGetStatus(COMM);
CEXT int
ioGetControl(COMM);
CEXT void
ioSetControl(COMM, int);
CEXT int
ioGetFIFO(COMM);
CEXT int
ioSetFIFO(COMM, int);
