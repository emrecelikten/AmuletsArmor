...
typedef struct _tag_midi_driver
{
    ...
    W32 ( cdecl
    far *lpfnDataFunction )( LPSTR, W32, W32 );

    // array of pointers to function for the system to call a driver with
    W32 ( cdecl
    far *lpfnFunction[_MIDI_DRV_FUNCTIONS] )(
    LPSTR, W32, W32 );
...
typedef struct _tag_midi_song
{
    ...
    VOID
    ( *pfnSongCallback )(HANDLE);

    // branch callback function
    W32
    ( *pfnBranchCallback )(HANDLE, BYTE, BYTE);

    // loop callback function
    W32
    ( *pfnLoopCallback )(HANDLE, BYTE, BYTE);

    // trigger callback function
    W32
    ( *pfnTriggerCallback )(HANDLE, BYTE, BYTE);
